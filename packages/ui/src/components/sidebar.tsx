...
function Sidebar({ className, children, ...props }: React.ComponentProps<"aside">) {
  ...
  return (
    <aside
      ...
      className={cn(
        "relative flex h-full flex-col border-r border-border bg-card text-card-foreground transition-all duration-300 ease-in-out",
        open ? "w-64" : "w-14",
        className,
      )}
      {...props}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden px-2 py-2", className)}
      {...props}
    />
  );
}
...
function SidebarGroupLabel({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      className={cn(
        "px-2 py-1 text-xs font-medium text-muted-foreground transition-all duration-200",
        !open && "sr-only",
        className,
      )}
      {...props}
...
function SidebarMenuButton({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center gap-2 rounded-none px-2 py-1.5 text-xs font-medium transition-colors outline-none hover:bg-accent hover:text-accent-foreground focus-visible:ring-1 focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent data-[active]:text-accent-foreground",
