...
function EditRoomPage() {
	...
	useEffect(() => {
		if (room) {
			reset({
				...
				name: room.name,
				status: room.status,
				equipmentList: (room.equipmentList as string[]) ?? [],
			});
		}
	}, [room, reset]);
	...
	function onSubmit(values: RoomFormValues) {
		mutation.mutate({ id: roomId, ...values });
	}
	...
	if (isLoading) {
		return (
			<div className="p-8">
				...
				<Skeleton className="h-80 max-w-lg rounded-xl" />
			</div>
		);
	}
	...
	if (!room) {
		return (
			<div className="p-8">
				<p className="text-on-surface-variant text-sm">Room not found.</p>
				<button
					type="button"
	...
	return (
		<div className="p-8">
			...
			<h1 className="mb-6 font-semibold text-2xl text-on-surface">Edit Room</h1>
